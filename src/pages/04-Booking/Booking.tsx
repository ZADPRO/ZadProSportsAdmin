...
const Booking: React.FC = () => {
  ...
  const [selectedBooking, setSelectedBooking]: any = useState<GroundBooking | null>(null);
  const [selectedData, setSelectedData]: any = useState(null);
  const [bookingData, setBookingData]: any = useState<GroundBooking[]>([]);
 
  const fetchBookingList = async () => {
    try {
      const response = await axios.get(`${import.meta.env.VITE_API_URL}/adminRoutes/listUserBookings`, {
        headers: {
          Authorization: "Bearer " + localStorage.getItem("JWTtoken"),
          "Content-Type": "application/json",
        },
      });
      const decryptedData = decrypt(response.data[1], response.data[0], import.meta.env.VITE_ENCRYPTION_KEY);
            ...
            <div className="ion-padding" slot="content">
              <div className="w-[100%]">
                <div className="w-[100%] flex justify-between text-[0.8rem] font-[poppins]">
                  <div style={{ fontWeight: 'bold', fontStyle: '', color: '', fontSize: '16px' }}>
                    Booked Date (
                  ...
                  <div style={{ fontWeight: 'bold', fontStyle: '',  fontSize: '15px' }}>
                ...
                <div
                  className={`mt-[10px] flex justify-between  fontSize: '15px' pb-[10px] text-[0.8rem] font-[poppins] ${(selectedBooking.payload?.refAddOns || []).length > 0
                    ? "border-b-[1px]"
                    : ""
                    }`}
                >
  ...
  return (
    <div className="p-4">
      ...
      <DataTable value={bookingData.result} scrollable
        showGridlines
        stripedRows 
        ...
        <Column
          header="Actions"
          body={(_, options: any) => (
            <button className="p-button p-button-sm p-button-info" onClick={() => openSidebar(options.rowIndex)}>
              View Details
            </button>
          )}
        />
